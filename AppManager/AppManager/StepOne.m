...
- (IBAction)nextButtonAction:(id)sender {
    ...
    if (self.businessNameField.text.length == 0) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Business name is required." message:@"Please try again." preferredStyle:UIAlertControllerStyleAlert];
        ...
        dispatch_async(aQueue, ^{
            ...
            [[Connection connection] didUpdateBusinessName:self.businessNameField.text];
            
            dispatch_async(dispatch_get_main_queue(), ^{
                [UIView animateWithDuration:0.3 animations:^{
                    _coverView.alpha = 0.0;
                }completion:^(BOOL finished){
                    [self.navigationController pushViewController:[StepTwo new] animated:YES];
                }];
            });
