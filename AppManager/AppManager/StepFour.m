...
@property (nonatomic) NSString *planSelected;

@end

@implementation StepFour
...
- (IBAction)nextButtonAction:(id)sender {
    ...
    dispatch_async(aQueue, ^{
        ...
        [[Connection connection] didUpdatePlan:self.planSelected];
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:0.3 animations:^{
                _coverView.alpha = 0.0;
            }completion:^(BOOL finished){
                StepFive *stepFive = [StepFive new];
                stepFive.whichOption = @"Monthly";
                if (self.annualOutlet.on) {
                    stepFive.whichOption = @"Annual";
                }
                
                [self.navigationController pushViewController:stepFive animated:YES];
            }];
        });
...
- (IBAction)monthlyAction:(id)sender {
    [self.annualOutlet setOn:NO animated:YES];
    self.planSelected = @"Month";
}

- (IBAction)annualAction:(id)sender {
    [self.monthlyOutlet setOn:NO animated:YES];
    self.planSelected = @"Annual";
}
@end
