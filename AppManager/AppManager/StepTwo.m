...
- (IBAction)nextButtonAction:(id)sender {
    ...
    if (self.firstNameField.text.length == 0 || self.lastNameField.text.length == 0) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"First and last name are required." message:@"Please try again." preferredStyle:UIAlertControllerStyleAlert];
        ...
        dispatch_async(aQueue, ^{
            ...
            [[Connection connection] didUpdateFirstName:self.firstNameField.text andLastName:self.lastNameField.text];
            
            dispatch_async(dispatch_get_main_queue(), ^{
                [UIView animateWithDuration:0.3 animations:^{
                    _coverView.alpha = 0.0;
                }completion:^(BOOL finished){
                    [self.navigationController pushViewController:[StepThree new] animated:YES];
                }];
            });
